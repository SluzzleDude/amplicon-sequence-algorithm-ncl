...
# Calculate the average abundance per treatment (Pesticide)
average_abundance <- data %>%
  group_by(Pesticide) %>%
  summarize(Average_Abundance = mean(Total_Abundance, na.rm = TRUE))

# Plot the average abundance per treatment
ggplot(average_abundance, aes(x = Pesticide, y = Average_Abundance, fill = Pesticide)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Average Abundance per Treatment",
       x = "Pesticide",
       y = "Average Total Abundance") +
